/**
 * Facebook Page data via Apify: apify/facebook-pages-scraper
...
export async function fetchFacebookPage(
  ...
  if (!items.length) {
    return {
      ...
      dataSources: ['Facebook via Apify (apify/facebook-pages-scraper)'],
    };
  }
  ...
  const result: FacebookResult = {
    found: true,
    pageName: d.title ? String(d.title) : d.name ? String(d.name) : null,
    followers: d.followers ? Number(d.followers) : d.followersCount ? Number(d.followersCount) : null,
    likes: d.likes ? Number(d.likes) : d.likesCount ? Number(d.likesCount) : null,
