...
import { isFirebaseConfigured, registerFirebase, sendPasswordResetFirebase, translateFirebaseError } from '../services/authService';
import { createVerificationCode, verifyCode } from '../services/emailVerification';
import { sendVerificationCodeEmail } from '../services/resendEmailService';
...
export default function LoginPage() {
  ...
  function resetForm() {
    ...
    setVerificationCode(''); setError(''); setSuccess('');
    setDemoCodeVisible(''); setShowPassword(false);
  }

  function switchMode(m: AuthMode) { resetForm(); setMode(m); }
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    try {
      ...
      else { setError(isFirebaseConfigured ? 'Email ou senha incorretos.' : 'Email ou senha incorretos. Verifique seus dados.'); }
    } catch (err: unknown) {
      const code = (err as { code?: string }).code;
      setError(code ? translateFirebaseError(code) : 'Erro inesperado. Tente novamente.');
  ...
  const handleRegisterEmail = async (e: FormEvent) => {
    ...
    try {
      ...
      const { demoMode } = await sendVerificationCodeEmail(email.trim(), code);
      if (demoMode) {
        setDemoCodeVisible(code);
        setSuccess(`Modo demo — código gerado.`);
      } else {
  ...
  const handleVerifyCode = (e: FormEvent) => {
    ...
    if (result.valid) {
      setError(''); setSuccess('Email verificado! Crie sua senha.');
      setMode('register_password');
    } else { setError(result.reason || 'Código inválido.'); }
  };
  ...
  const handleSetPassword = async (e: FormEvent) => {
    ...
    try {
      if (isFirebaseConfigured) {
        await registerFirebase(email.trim(), password, name.trim());
        // onAuthStateChanged in App.tsx will update the store
        navigate('/dashboard');
      } else {
        // Demo mode: just log in with demo mode (any @ivoire.ag)
        const ok = await login(email.trim(), password);
        if (ok) navigate('/dashboard');
        else setError('Erro ao criar conta demo.');
      }
    } catch (err: unknown) {
      const code = (err as { code?: string }).code;
      setError(code ? translateFirebaseError(code) : 'Erro ao criar conta. Tente novamente.');
    } finally { setLoading(false); }
  };
  ...
  const handleForgot = async (e: FormEvent) => {
    ...
    try {
      if (isFirebaseConfigured) {
        ...
        setSuccess('Para redefinir sua senha, contate o administrador Ivoire.');
      }
    } catch (err: unknown) {
      const code = (err as { code?: string }).code;
      setError(code ? translateFirebaseError(code) : 'Erro ao enviar email de recuperação.');
    } finally { setLoading(false); }
  };
  ...
  const modeConfig: Record<AuthMode, { title: string; subtitle: string; action: string }> = {
    ...
    forgot: { title: 'Recuperar senha', subtitle: isFirebaseConfigured ? 'Informe seu email para receber o link de recuperação.' : 'Contate o administrador Ivoire.', action: 'Enviar email de recuperação' },
  };
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px 64px' }}>
        <div style={{ width: '100%', maxWidth: '400px' }}>
          <div className="font-montserrat" style={{ fontSize: '11px', fontWeight: 700, color: '#FFFF02', letterSpacing: '3px', marginBottom: '12px' }}>
            {mode === 'login' ? 'ACESSO AO SISTEMA' : mode.startsWith('register') ? 'NOVO CADASTRO' : 'RECUPERAR SENHA'}
          </div>

          {/* Step indicator for registration */}
          {mode.startsWith('register') && (
            <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', alignItems: 'center' }}>
              {(['register_email', 'register_verify', 'register_password'] as AuthMode[]).map((step, idx) => {
                const steps: AuthMode[] = ['register_email', 'register_verify', 'register_password'];
                const currentIdx = steps.indexOf(mode as AuthMode);
          ...
          {demoCodeVisible && (
            <div style={{ marginBottom: '16px', padding: '14px 16px', background: 'rgba(255,153,0,0.08)', border: '1px solid rgba(255,153,0,0.3)', borderRadius: '6px' }}>
              ...
              <p style={{ fontSize: '11px', color: '#c9c9c9', fontFamily: 'Arvo, serif', margin: '6px 0 0' }}>
                Configure VITE_RESEND_API_KEY para envio real de emails.
              </p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {/* Email */}
            {['login', 'register_email', 'forgot'].includes(mode) && (
              <div>
                <label style={{ display: 'block', fontFamily: 'Montserrat', fontSize: '10px', fontWeight: 700, color: '#999', letterSpacing: '1.5px', marginBottom: '7px' }}>EMAIL</label>
                <input type="email" className="ivoire-input" value={email} onChange={(e) => setEmail(e.target.value)}
            ...
            {mode === 'register_email' && (
              <div>
                <label style={{ display: 'block', fontFamily: 'Montserrat', fontSize: '10px', fontWeight: 700, color: '#999', letterSpacing: '1.5px', marginBottom: '7px' }}>NOME COMPLETO</label>
                <input type="text" className="ivoire-input" value={name} onChange={(e) => setName(e.target.value)}
            ...
            {mode === 'register_verify' && (
              <div>
                <label style={{ display: 'block', fontFamily: 'Montserrat', fontSize: '10px', fontWeight: 700, color: '#999', letterSpacing: '1.5px', marginBottom: '7px' }}>CÓDIGO DE VERIFICAÇÃO</label>
                <input type="text" className="ivoire-input"
                  value={verificationCode} onChange={(e) => setVerificationCode(e.target.value.toUpperCase())}
                  placeholder="XXXXXX" autoFocus autoComplete="one-time-code" maxLength={6} disabled={loading}
                  style={{ letterSpacing: '6px', fontSize: '18px', textAlign: 'center', fontFamily: 'Bebas Neue, cursive' }}
                />
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px' }}>
                  <span style={{ fontSize: '11px', color: '#e6e1e1', fontFamily: 'Arvo, serif' }}>Expira em 15 minutos</span>
            ...
            {/* Password */}
            {['login', 'register_password'].includes(mode) && (
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  <input type={showPassword ? 'text' : 'password'} className="ivoire-input"
            ...
            {mode === 'register_password' && (
              <div>
                <label style={{ display: 'block', fontFamily: 'Montserrat', fontSize: '10px', fontWeight: 700, color: '#999', letterSpacing: '1.5px', marginBottom: '7px' }}>CONFIRMAR SENHA</label>
                <input type={showPassword ? 'text' : 'password'} className="ivoire-input"
            ...
            <button type="submit" className="btn-primary" disabled={loading}
              style={{ width: '100%', justifyContent: 'center', padding: '14px 24px', fontSize: '13px', marginTop: '4px' }}>
              {loading
          ...
          {mode === 'login' && !isFirebaseConfigured && (
            <div style={{ marginTop: '28px', padding: '14px 16px', background: 'rgba(255,255,2,0.04)', border: '1px solid rgba(255,255,2,0.1)', borderRadius: '6px' }}>
              <div style={{ fontSize: '9px', fontFamily: 'Montserrat', fontWeight: 700, color: '#FFFF02', letterSpacing: '1.5px', marginBottom: '7px' }}>ACESSO DEMO</div>
              <p style={{ fontFamily: 'Arvo, serif', fontSize: '12px', color: '#c9c9c9', margin: 0, lineHeight: 1.6 }}>
                ...
                <br />ou qualquer email <span style={{ color: '#FFFF02' }}>@ivoire.ag</span>
              </p>
            </div>
          )}

          {mode === 'login' && isFirebaseConfigured && (
            <div style={{ marginTop: '28px', padding: '14px 16px', background: 'rgba(0,204,102,0.04)', border: '1px solid rgba(0,204,102,0.15)', borderRadius: '6px' }}>
              <div style={{ fontSize: '9px', fontFamily: 'Montserrat', fontWeight: 700, color: '#00cc66', letterSpacing: '1.5px', marginBottom: '6px' }}>AUTENTICAÇÃO SEGURA</div>
              <p style={{ fontFamily: 'Arvo, serif', fontSize: '12px', color: '#c9c9c9', margin: 0, lineHeight: 1.5 }}>
                Firebase Auth ativo. Use seu email <span style={{ color: '#FFFF02' }}>@ivoire.ag</span> cadastrado.
              </p>
            </div>
          )}
